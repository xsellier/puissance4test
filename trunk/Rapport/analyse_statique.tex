...
Dans cette partie, nous avons représenté des fonctions clés de notre
programme à l'aide de CFA. Ceux-ci nous servent à effectuer une
série de tests statiques des fonctions critiques du puissance4. Nous trouverons dans l'ordre: la fonction principale de notre moteur de jeu,
...
\texttt{IA} sont amputés des fonctions auquelles ils font appel. On considèrera dès
lors que ces appels effectuent la fonction pour laquelle ils sont
écrits et ne provoquent pas de bug dans le programme. Ces fonctions sont
...
jouée est comprise entre les valeurs de la grille et si la colonne dans
...
difficile car celui de l'IA facile est le même à la différence près qu'il
...
pertinentes pour être intégrées dans le rapport. Nous les avons
néanmoins fournies en consultation dans les pages suivantes.\\ 
...
couverture suffisant pour être pertinentes. Cela a grandement facilité
la mise en place d'une stratégie de test efficace des différents
modules du puissance4. Il est aussi intéressant de mentioner le fait
que la création du CFA \texttt{vérification\_choix()} nous a permis de
corriger une erreur d'écriture de fonction. En effet nous avions codé
cette fonction de telle façon qu'elle nous renvoyait toujours true,
même en passant des valeurs qui sortaient des limites de la
grille. Le CFA a mis en valeur ce défaut de conception et nous a
