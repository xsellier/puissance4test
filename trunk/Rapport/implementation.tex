...
Nos différentes classes de test vont dériver de \textbf{junit.framework.TestCase}. Elles auront une méthode setUp(), exécutée avant chaque méthode de test, pour initialiser les tests et une méthode tearDown(), exécutée après chaque méthode de test , pour relâcher les données.\newline
Les méthodes de test n'auront pas de paramètres et retournent \textbf{void}. Leur nom commencera par \textbf{testXXX}.\newline
A l'intérieur des méthodes de test, on effectuera les actions de test souhaitées, et on
vérifiera qu'elles se passent correctement en utilisant des méthodes \textbf{assertXXX}.\newline
...
Un \textbf{test aux limites} sera mis en place dans cette méthode : En plus de tester la possibilité que toutes les cases de la grille soient remplies, nous avons testé le rajout de valeurs à des positions \textit{hors limite}.\newline
Exemple :\newline
...
Tester la classe \textbf{HumanPlayer} se limitera à tester la méthode \textbf{play(DataStructure , GUI gui)}.\newline
...
\item Qu'une position donnée est jouable en remplissant la grille aléatoirement mais de manière à ce qu'une position soie jouable/non jouable.  
...
Nous devons élaborer des tests fonctionnels pour la classe \textbf{GameEngine} afin de vérifier la conformité du déroulement du jeu et les données retournées par la classe.\newline
Les méthodes seront \textbf{testHuman2WinHuman1()}, \textbf{testCpu1WinHuman()} et \textbf{testCpu2WinHuman()} qui lancent respectivement des confrontations Humain/Humain, Cpu1/Humain et Cpu2/Humain.\newline
...
\begin{center}
\textbf{assertFalse("2 possibilités : $\backslash$n C'est le joueur humain numéro 1 qui a gagné !!!$\backslash$n" +
...
\begin{center}
\textbf{assertFalse("C'est l'humain qui a gagné !", game.getWinPlayer());}
\end{center}
...
Une dernière méthode \textbf{testModes()} va tester la mise-à-jour des modes de jeu.\newline
Exemple :\newline
\textbf{assertTrue(game.initMode(0));}
\textbf{assertTrue(game.getMode() == 0);}
\textbf{assertFalse(game.getMode() == 1);}
\textbf{assertFalse(game.getMode() == 2);}
