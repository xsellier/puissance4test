...
Notre première méthode est un constructeur. Elle attribue donc un \texttt{mode}
...
gagner au prochain coups, et jouera la position 2.
...
Nous allons reprendre les exemples de la partie implémentation IaForInRow en ajoutant comme prévu de nouveaux cas.
...
la grille de jeu. Dans le rapport l'ensemble des tests se fera à
l'aide d'une IA en mode difficile. L'IA facile ayant le même
comportement avec des stratégies en moins (méthodes en moins), il n'est pas nécessaire de la
tester. Certains tests sont quand même présents dans \texttt{Junit}
sur l'ordinateur en mode facile avec des grilles non
conventionnelles. De même la validité des accesseurs a été testé. Nous avons choisis ici de présenter que les résultats significatifs.

L'ordinateur joue de manière séquentille, si bien qu'il est possible
de calculer chacun de ses coups en connaissant l'état de sa grille
\texttt{playable[i]}.

Tout d'abord nous avons vérifié que \texttt{initialize(DataStructure grid, int difficulty)} initialise correctement l'objet \texttt{IaForInRow}. C'est à dire que \texttt{CpuGrid} soit le même Objet que la grille \texttt{grid} passée en référence, et que le mode de difficulté soit correct.
...
Avec unegrille vide nous devons vérifier que l'ordinateur ne met pas en place de stratégie, donc que 
...
interprété par le GameEngine, car il ne se produiera jamais (voir
analyse statique).
...
\item Qu'une position donnée est jouable en remplissant la grille aléatoirement mais de manière à ce qu'une position soit jouable/non jouable.  
