...
Nous avons testé des ajouts de valeurs prises de manière aléatoire
...
Un \textbf{test aux limites} a été mis en place dans cette méthode : En plus de tester la possibilité que toutes les cases de la grille soient remplies, nous avons testé le rajout de valeurs à des positions \textit{hors limite}.\newline
Exemple :\newline
...
Tester la classe \textbf{HumanPlayer} se limite à tester la méthode \textbf{play(DataStructure , GUI gui)}.\newline
Nous avons testé que la valeur de la position jouée n'est pas nulle et qu'elle est comprise entre 0 et la largeur de la grille(7), celà s'écrit :\newline
